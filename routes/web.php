...
Route::group(
    ...
    function () {
		...
		Route::prefix('project_managers')->name('project_managers.')->group(function () {
			Route::get('gantt', 'ProjectManagerController@gantt')->name('gantt.index');

			Route::get('{project_manager}/gantt', 'ProjectManagerController@gantt')->name('gantt.show');
			Route::get('{project_manager}/cards', 'ProjectManagerController@cards')->name('cards');
			Route::get('{project_manager}/report', 'ProjectManagerController@report')->name('report');
		});
		...
		Route::get('/ventas/cliente_registers', 'Ventas_registroController@clientes_registers')->name('ventas.clientes_registers');
	...
	// COMPROBANTES (NEW VISTAS V2.1)
		...
		Route::get('/comprobantes/factura_registers', 'ComprobantesVentasController@factura_registers')->name('comprobantes.factura_registers');
...
Route::post('/garantia/informe-tecnico/descargar-multiple',[GarantiaInformeTecnicoController::class, 'downloadMultiplePDFs'])
    ...
    Route::get('/debug-zip-pdf', function() {
    try {
        // Obtener una factura real
        $facturacion = \App\Facturacion_m::first();

        if (!$facturacion) {
            return 'No hay facturas en la BD';
        }

        $facturacion_registro = \App\Facturacion_registro_m::where('facturacion_m_id', $facturacion->id)->get();
        $igv = \App\Igv::first();
        $banco = \App\Banco::where('estado', 0)->get();
        $banco_count = \App\Banco::where('estado', '0')->count();
        ...
        // Probar diferentes métodos de obtener el PDF
        $method1 = $pdf->output();
        $method2 = null;
        try {
            ob_start();
            $pdf->stream();
            $method2 = ob_get_clean();
        } catch (\Exception $e) {
            $method2 = 'Error: ' . $e->getMessage();
        }

        // Crear ZIP con método 1
        $zipPath1 = storage_path('app/temp/test_method1.zip');
        $zip1 = new \ZipArchive();
        $zip1->open($zipPath1, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
        $zip1->addFromString('factura_method1.pdf', $method1);
        $zip1->close();
        unset($zip1);

        // Crear ZIP con método 2
        $zipPath2 = storage_path('app/temp/test_method2.zip');
        $zip2 = new \ZipArchive();
        $zip2->open($zipPath2, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
        if (!is_string($method2) || strpos($method2, 'Error') === 0) {
            $zip2->addFromString('error.txt', $method2);
        } else {
            $zip2->addFromString('factura_method2.pdf', $method2);
        }
        $zip2->close();
        unset($zip2);

        // Verificar ambos ZIPs
        $info = [
            'method1_output' => [
                'size' => strlen($method1),
                'first_bytes' => bin2hex(substr($method1, 0, 10)),
                'zip_exists' => file_exists($zipPath1),
                'zip_size' => file_exists($zipPath1) ? filesize($zipPath1) : 0,
            ],
            'method2_stream' => [
                'size' => is_string($method2) ? strlen($method2) : 0,
                'first_bytes' => is_string($method2) ? bin2hex(substr($method2, 0, 10)) : 'N/A',
                'zip_exists' => file_exists($zipPath2),
                'zip_size' => file_exists($zipPath2) ? filesize($zipPath2) : 0,
            ],
            'zip1_path' => $zipPath1,
            'zip2_path' => $zipPath2,
        ];

        // Verificar signature de ambos ZIPs
        if (file_exists($zipPath1)) {
            $h1 = fopen($zipPath1, 'rb');
            $info['method1_output']['zip_signature'] = bin2hex(fread($h1, 4));
            fclose($h1);

            $z1 = new \ZipArchive();
            $info['method1_output']['can_reopen'] = $z1->open($zipPath1) === true;
            $z1->close();
        }

        if (file_exists($zipPath2)) {
            $h2 = fopen($zipPath2, 'rb');
            $info['method2_stream']['zip_signature'] = bin2hex(fread($h2, 4));
            fclose($h2);

            $z2 = new \ZipArchive();
            $info['method2_stream']['can_reopen'] = $z2->open($zipPath2) === true;
            $z2->close();
        }
