...
class CobranzasBoletasMExport implements FromQuery, WithHeadings, WithMapping, WithEvents
{
    /**
    ...
    public function map($f): array
    {
        ...
        if ($f->nota_credito != 0) {
            $nota_cred = Nota_Credito::where('boleta_m_id', $f->id)->first();
            $codigo_nc = $nota_cred->codigo_n_c;
        }
        if ($f->nota_debito != 0) {
            $nota_deb = Nota_Debito::where('boleta_m_id',  $f->id)->first();
