...
class CobranzasFacturasExport implements FromQuery, WithHeadings, WithMapping, WithEvents
{
    protected ?array $ids;
    ...
    public function map($f): array
    {
        ...
        if($f->nota_credito != 0){
            $nota_cred = Nota_Credito::where('facturacion_id', $f->id)->first();
            $codigo_nc = $nota_cred->codigo_n_c;
        }
        if($f->nota_debito != 0){
            $nota_deb = Nota_Debito::where('facturacion_id',  $f->id)->first();
