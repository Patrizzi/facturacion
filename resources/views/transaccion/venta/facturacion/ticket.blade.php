    ...
    <div class="ticket">
        ...
        <header>
            <div class="header-box box-outline">
                <h1>Factura Electrónica<br>{{ $facturacion->codigo_fac }}</h1>
            </div>
            <hr>
            <div class="info-section">
                ...
                @if (isset($facturacion->cliente))
                    ...
                    <p><span class="bold">{{ $facturacion->cliente->documento_identificacion }}:</span>
                        {{ $facturacion->cliente->numero_documento }}</p>
                @else
                    <p><span class="bold">CLIENTE:</span> {{ $facturacion->cotizador->cliente->nombre }}</p>
                @endif
                <p><span class="bold">FECHA EMISION:</span> {{ $facturacion->fecha_emision }}</p>
                <p><span class="bold">FECHA VENCIMIENTO:</span> {{ $facturacion->fecha_vencimiento }}</p>
            </div>
            <hr>
            <table class="tabla-condicion">
                <tr>
                    <td><span class="bold">Condición:</span> {{ $facturacion->forma_pago->nombre }}</td>
                ...
                <tbody>
                    ...
                    @foreach ($facturacion_registro as $item)
                        <tr>
                            ...
                            <td class="col-pu">{{ number_format((float) $item->precio_unitario_comi, 2) }}</td>
                            <td class="col-imp">{{ number_format((float) $item->precio_unitario_comi * (float) $item->cantidad, 2) }}</td>
                        </tr>
                    @endforeach
                    <tr>
                        <td colspan="4">
