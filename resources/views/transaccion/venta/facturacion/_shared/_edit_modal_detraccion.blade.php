...
<div class="modal fade" id="modal_detraccion" role="dialog" aria-labelledby="exampleModalCenterTitle">
                            ...
                            <div class="row">
                                <div class="col-sm-8">
                                    ...
                                    <select class="select2_tipodetrac ipt_detrac" name="tipo_detraccion"
                                        ...
                                        @if (isset($facturacion->detracciones))
                                            @foreach ($tipo_detraccion as $detra)
                                                <option value="{{ $detra->id }}"
                                                    @if ($detra->id == $facturacion->detracciones->id_cod_tipo_detraccion) selected @endif>
                                                    {{ $detra->codigo }} -
                                                    {{ $detra->descripcion }}</option>
                                            @endforeach
                                        @else
                                            @foreach ($medio_pago_detraccion as $detra)
                                                <option value="{{ $detra->id }}">
                                                    {{ $detra->codigo }} -
                                                    {{ $detra->descripcion }}</option>
                                            @endforeach
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="text" class="form-control ipt_detrac" name="porcentaje_detraccion"
                                        id="porcentaje_detc" value="{{$facturacion->detracciones?->porcentaje_detraccion ?? '' }}">
                                    <div class="detracc_campo_required">
                            ...
                            <div class="row">
                                <div class="col-sm-8">
                                    ...
                                    <select class="select2_mediopago ipt_detrac" name="medio_pago_detraccion"
                                        ...
                                        @foreach ($medio_pago_detraccion as $m_pago)
                                            <option value="{{ $m_pago->id }}" @if ($m_pago->id == $facturacion->detracciones?->id_cod_medio_pago) selected @endif>
                                                {{ $m_pago->codigo }} -
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control ipt_detrac" name="total_detraccion"
                                            id="tota_detra" placeholder="0.00" readonly value="{{$facturacion->detracciones?->monto_detraccion ?? '' }}">
                                    </div>
