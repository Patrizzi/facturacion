...
<script>
    ...
    function ajax(a) {
        ...
        // var cont_origi = Number("{{ count($boleta->registros) }}");
    ...
    function search_multiple(busqueda) {
        ...
        $.ajax({
            ...
            data: {
                ...
                'fecha_tipo_cambio': "{{ $boleta->fecha_emision }}",
            },
            success: function(msg) {
    ...
    $(".add_pago").on('click', function() {
        ...
        var fecha_min = "$boleta->fecha_emision }}";
        var total = document.getElementById('total_final').value;
        var data = `
    ...
    let status =
        "@if ($boleta->moneda->principal == 1) 0 @else 1 @endif";
    let total_val = 0;
    let completed = 0;
