                                                    ...
                                                    <table class="table table-striped table-bordered dataTables-example-facturam">
                                                        ...
                                                        <tbody>
                                                                ...
                                                                <tr class="gradeX">
                                                                    ...
                                                                    <td>{{$factura_m->fecha_emision}}</td>
                                                                    <td>{{$factura_m->cliente->nombre}}</td>
                                                                    <span hidden>{{$subtotal = $factura_m->op_gravada + $factura_m->op_inafecta + $factura_m->op_exonerada }} </span>
                                                                    <td>{{$factura_m->moneda->simbolo}} {{number_format(round(($subtotal+($factura_m->op_gravada*$igv->renta/100)),2),2)}}</td>
                                                                    <td>
                                                                        <form method="POST" action="{{route('nota-credito.motivo')}}">
                                                                            @csrf
                                                                            <input type="hidden" name="factura_manual_id" value="{{$factura_m->id}}">
