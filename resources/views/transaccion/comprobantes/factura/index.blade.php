...
@section('content')
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered dataTables-example-factura" style="min-width: 982px">
                                            ...
                                            <tfoot>
                                                <tr>
                                                    <th colspan="7"></th>
                                                    <th colspan="2" class="total-columna">Total: 0</th>
    ...
    $(document).ready(function() {
        ...
        $(document).on('submit', '.form-enviar-email', function(e) {
            ...
            $.ajax({
                url: "{{ route('factura.enviar-correo-directo', '') }}/" + facturaId,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        swal({
            ...
            $('#btn-correo-filtrado').on('click', function(e) {
                ...
                swal({
                    ...
                    // Enviar por AJAX
                    $.ajax({
                        url: '{{ route('envioCorreo.factura.multiple') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            email: inputValue,
    ...
    <script>
        ...
        function enviarNotaAjax(id, nota, isDelete = false) {
            ...
            $.ajax({
                url: "{{ route('facturacion.guardar_nota', ':id') }}".replace(':id', id),
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
