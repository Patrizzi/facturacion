...
<body class="white-bg" onLoad="setTimeout('cerrar()',1*1000)">
    ...
    @foreach($facturasData as $index => $facturaData)
        @php
            ...
            // Calcular totales AQUÍ, fuera del bucle de productos
                        ...
                        <div class="col-sm-6" align="center">
                            <div class="form-control">
                                <div align="left">
                                    ...
                                    @if(isset($factura->cliente_id)){{$factura->cliente->nombre}}
                                    @else{{$factura->cotizacion->cliente->nombre}}
                                    @endif <br>
                                    <strong>R.U.C:</strong>
                                    @if(isset($factura->cliente_id)){{$factura->cliente->numero_documento}}
                                    @else{{$factura->cotizacion->cliente->numero_documento}}
                                    @endif <br>
                                    <strong>Dirección:</strong>
                                    @if(isset($factura->cliente_id)){{$factura->cliente->direccion}}
                                    @else{{$factura->cotizacion->cliente->direccion}}
                                    @endif <br>
                                    <strong>Condiciones de Pago:</strong>
                                    @if(isset($factura->cliente_id)){{$factura->forma_pago->nombre }}
                                    @else{{$factura->cotizacion->forma_pago->nombre }}
                                    @endif  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <strong>Tipo de Moneda:</strong>
                                    @if(isset($factura->cliente_id)){{$factura->moneda->nombre }}
                                    @else{{$factura->cotizacion->moneda->nombre }}
                                    @endif <br>
                                </div>
                            </div>
                        </div>
                            ...
                            @foreach ($cuotas as $cuota)
                                ...
                                <div class="col-sm-3">
                                    <div class="form-control">
                                        ...
                                        {{$factura->moneda->simbolo}} {{number_format($monto_total_det, 2)}}
                                        <br>
