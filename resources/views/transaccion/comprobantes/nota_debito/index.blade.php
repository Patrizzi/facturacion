...
@section('content')
                                ...
                                <ul class="nav nav-tabs" role="tablist" style="align-items: center;border-bottom: 0px !important;">
                                    <div class="nav nav-custom" style="min-width: 1450px;overflow-y: hidden;overflow-x: auto;">
                                        ...
                                        <ul class="ml-auto d-flex" style="gap: 10px; align-items: center;z-index: 20;position: fixed;right: 40px">
                                            {{-- ALMACEN --}}
                                            @canany(['nota_debito.crear_factura','nota_debito.crear_factura_m','nota_debito.crear_boleta','nota_debito.crear_boleta_m'])
                                                <span class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu animated fadeInRight m-t-xs">
                                                        ...
                                                        @canany(['nota_debito.crear_factura','nota_debito.crear_factura_m'])
                                                            <a class="btn btn-w-m btn-link"
                                                                href="{{route('nota-debito.create')}}">Factura</a>
                                                        @endcan
                                                        @canany(['nota_debito.crear_boleta','nota_debito.crear_boleta_m'])
                                                            <a class="btn btn-w-m btn-link"
        ...
        var coti_table = $('.dataTables-example-nota-debito').DataTable({
            ...
            "columnDefs": [{
                ...
                },
                {
                    ...
                    'orderable': false,
                    'render': function(data, type, full, meta) {
                        const notaDebitoId = full[0];
                        const codigonotaDebito = full[2];
                        ...
                        return `
                            <div style="display: inline-block; white-space: nowrap;">
                                <!-- Contenedor Correo -->
                                <div class="email-container" data-id="${notaDebitoId}"
                                    ...
                                    <div class="email-form" data-id="${notaDebitoId}"
                                        ...
                                        <form class="form-enviar-email" data-nota-id="${notaDebitoId}" style="padding: 10px;">
                                            @csrf
                                            ...
                                            <div class="emails-adicionales-${notaDebitoId}"></div>
                                            <button type="button" class="btn-agregar-email btn btn-info btn-xs" data-id="${notaDebitoId}"
                                ...
                                <!-- Contenedor WhatsApp -->
                                <div class="wsp-container" data-id="${notaDebitoId}"
                                    ...
                                    <div class="wsp-form" data-id="${notaDebitoId}"
                                        style="position: absolute; top: 100%; right: 0; margin-top: 5px; height: 0px;
                                        overflow: hidden; transition: height .4s; background: white;
                                        box-shadow: 0px 0px 5px rgba(0,0,0,0.3); border-radius: 4px;
                                        ...
                                        <form action="{{ route('agregado.whatsapp_send') }}" method="post" target="_blank" style="padding: 10px;">
                                            ...
                                            <input type="hidden" name="url" value="{{ route('nota_debito.pdf', '') }}/${notaDebitoId}?archivo=" />
                                            <input type="hidden" name="name_sin_cambio" value="NotaDébito_${codigonotaDebito}" />
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-agregar-email', function() {
            const notaDebitoId = $(this).data('id');
            const container = $(`.emails-adicionales-${notaDebitoId}`);
            ...
            const form = $(`.email-form[data-id="${notaDebitoId}"]`);
        ...
        $(document).on('submit', '.form-enviar-email', function(e) {
            ...
            const form = $(this);
            const notaDebitoId = form.data('nota-id');
            const button = form.find('button[type="submit"]');
            const originalHtml = button.html();
            const emailFormContainer = $(`.email-form[data-id="${notaDebitoId}"]`);
            ...
            $.ajax({
                url: "{{ route('notaDebito.enviar-correo-directo', '') }}/" + notaDebitoId,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        // Resetear formulario
                        form[0].reset();
                        $(`.emails-adicionales-${notaDebitoId}`).empty();
            ...
            swal({
                ...
                $.ajax({
                    ...
                    success: function(blob) {
                        ...
                        a.href = url;
                        a.download = `Notas de Crédito_${new Date().toISOString().slice(0,10)}.xlsx`;
        ...
        $('#btn-descargar-filtrado').on('click', function(e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    var url = '{{ route("notaD.download.multiple") }}';
                    var params = new URLSearchParams();
        ...
        $('#btn-correo-filtrado').on('click', function(e) {
            ...
            if (allSelectedIds.length === 0) {
                return swal({
                    title: "Sin selección",
                    text: "Por favor, selecciona al menos una nota de débito para enviar por correo.",
                    type: "warning",
                    confirmButtonText: "Entendido",
                    confirmButtonColor: "#1a3bb3"
                });
            ...
            swal({
                ...
                text: `Ingresa el correo electrónico para enviar ${allSelectedIds.length} nota(s) de débito:`,
                type: "input",
                showCancelButton: true,
                ...
                swal({
                    title: "Enviando...",
                    text: `Procesando ${allSelectedIds.length} notas(s) de débito. Por favor espera...`,
                    showConfirmButton: false,
                ...
                // Enviar por AJAX
                $.ajax({
                    url: '{{ route('envioCorreo.notaDebito.multiple') }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        email: inputValue,
                    ...
                    success: function(response) {
                        if (response.success) {
                            swal({
                                title: "¡Enviado!",
                                text: response.message || `Se han enviado ${allSelectedIds.length} notas(s) de débito por correo`,
                                type: "success",
                                timer: 3000,
        ...
        $('#btn-whatsapp-filtrado').on('click', function(e) {
            ...
            swal({
                ...
                text: `Ingresa el número de WhatsApp para enviar ${allSelectedIds.length} nota(s) de débito(s):`,
                type: "input",
                showCancelButton: true,
                closeOnConfirm: false,
                confirmButtonText: "Enviar",
                ...
                swal({
                    title: "Procesando...",
                    text: "Enviando notas de débito por WhatsApp",
                    type: "info",
                    showConfirmButton: false,
                    allowOutsideClick: false
                });
                ...
                const form = $('<form>', {
                    action: '{{ route('envioWhatsapp.notaDebito.multiple') }}',
                    method: 'POST',
                    target: '_blank',
                    style: 'display:none;'
                });
                ...
                setTimeout(() => {
                    swal({
                        title: "¡Enviado!",
                        text: `Se han enviado ${allSelectedIds.length} nota(s) de débito(s) por WhatsApp`,
                        type: "success",
                        timer: 3000,
                        showConfirmButton: true
                    });
