...
<style>
    ...
    <script>
         ...
         * ========================= */
        $(document).on('ifChecked ifUnchecked change', '.dataTables-example-guia-remision thead input[type="checkbox"]',
            function(event) {
                if (isUpdatingCheckboxes) return;
                closeWhatsappPanels();
                closeEmailPanels();
        ...
        $(document).on('submit', '.form-enviar-email', function(e) {
            ...
            toastr.info('<i class="fa fa-spinner fa-spin"></i> Enviando correo, no cierre esta pestaña...',
                'Procesando', {
                    timeOut: 0,
                    extendedTimeOut: 0,
                    closeButton: false,
        ...
        coti_table.on('draw.dt', function() {
    ...
    <script>
        ...
        function enviarNotaAjax(id, nota, isDelete) {
            $.ajax({
                url: "{{ route('remision_m.guardar_nota', ':id') }}".replace(':id', id),
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                ...
                success: function(response) {
                    if(response.success){
                        ...
                        setTimeout(function(){
                            coti_table.ajax.reload(null, false);
                        }, 500);
                    } else {
