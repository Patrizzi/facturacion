...
@section('content')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#tab_debito_env').addClass('active');
            // CHEK
            $('.i-checks-debito_env_all').iCheck({
            ...
            var table_debito_env = $('.dataTables-example3').DataTable({
                "serverSide": true,
                "ajax": {
                    url: "{{ route('facturacion_electronica.list_nota_dedito_env') }}",
                    method: "get",
                    data: function(d) {
                        // Aquí añades los parámetros que quieres enviar junto con la petición AJAX
                        d.daterange = $('#dateranger_debito')
        ...
        $('tbody input[class="i-checks-debito_env"]').on('ifChanged', function(event) {
            var table = $(this).closest('table'); // Limita el control a la tabla visible
            if (table.find('tbody input[class="i-checks-debito_env"]').filter(':checked').length === table
                .find(
                    'tbody input[class="i-checks-debito_env"]').length) {
                table.find('thead input[class=i-checks-debito_env_all"]').iCheck('check');
            } else {
                table.find('thead input[class=i-checks-debito_env_all"]').iCheck('uncheck');
            }
        });
    </script>
    <script>
        ...
        function download_pdf_select() {
            var checks = $('input[class=i-checks-debito_env]:checkbox:checked');
