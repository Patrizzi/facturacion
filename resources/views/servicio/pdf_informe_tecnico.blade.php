...
<body>
    ...
    @if (!empty($salida) && count($salida) > 0)
        @foreach ($salida as $detalle_s)
            ...
            @if(isset($imagenesProducto[$detalle_s->id]) && ($imagenesProducto[$detalle_s->id]->foto || $imagenesProducto[$detalle_s->id]->descripcion))
                <div class="imagen-container">
                    <div class="imagen-foto">
                        @if(isset($imagenesProducto[$detalle_s->id]->foto) && !empty($imagenesProducto[$detalle_s->id]->foto))
                        <img src="{{ $imagenesProducto[$detalle_s->id]->foto }}" alt="Imagen del producto">
                        @endif
                    </div>
                    <div class="imagen-descripcion">
                        @if(isset($imagenesProducto[$detalle_s->id]->descripcion) && !empty($imagenesProducto[$detalle_s->id]->descripcion))
                            <p><strong>Descripción de imagen:</strong> {{ $imagenesProducto[$detalle_s->id]->descripcion }}</p>
                        @endif
                    </div>
                </div>
            @endif
