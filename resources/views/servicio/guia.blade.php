    ...
    <div id="seccion1" class="contenido activo">
        ...
        <div class="Div-agregar">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h2 id="titulo-guia-servicio" class="mb-0">Guías de Servicio</h2>
                @if (!$buttonDisabled === true)
                <button id="btn-agregar-guia" type="button" class="btn btn-primary" data-toggle="modal" data-target="#productoModal">
            ...
            <div id="productoModal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="overflow: visible;">
                <div class="modal-dialog modal-lg modal-dialog-centered" role="document" style="overflow: visible;">
                    <div class="modal-content">
                        <form id="producto-form" method="POST" action="{{ route('servicio.guia.productos.store', ['guia_id' => $guia->id]) }}" class="modal-content">
                            @csrf
        ...
        <div class="wrappercontenedor">
            ...
            <div class="containercontenedor">
                ...
                <div class="input-groupcontenedor">
                    ...
                    <input type="text" id="dni" name="dni" class="input-fieldcontenedor" value="{{ $guia->cliente->numero_documento ?? 'No disponible' }}" readonly>

                    <label for="nombre" class="input-labelcontenedor">Nombre:</label>
                    <input type="text" id="nombre" name="nombre" class="input-fieldcontenedor" value="{{ $guia->cliente->nombre ?? 'No disponible' }}" readonly>
                </div>

                <div class="input-groupcontenedor">
                    <label for="direccion" class="input-labelcontenedor">Dirección:</label>
                    <input type="text" id="direccion" name="direccion" class="input-fieldcontenedor full-widthcontenedor" value="{{ $guia->cliente->direccion ?? 'No disponible' }}" readonly>
                </div>

                <div class="input-groupcontenedor">
                    <label for="contacto" class="input-labelcontenedor">Email:</label>
                    <input type="email" id="contacto" name="contacto" class="input-fieldcontenedor" value="{{ $guia->cliente->email ?? 'No disponible' }}" readonly>

                    <label for="telefono" class="input-labelcontenedor">Teléfono:</label>
                    <input type="number" id="telefono" name="telefono" class="input-fieldcontenedor" value="{{ $guia->cliente->telefono ?? 'No disponible' }}" readonly>
                </div>
            </div>
            ...
            <div class="containercontenedor" style="align-self: flex-end;">
                ...
                <div class="input-groupcontenedor">
                    ...
                    <input type="text" id="recepcionista" name="recepcionista" class="input-fieldcontenedor" value="{{ $guia->recepcionista ?? 'No asignado' }}" readonly>

                    <label for="fecha_ingreso" class="input-labelcontenedor">Fecha Ingreso:</label>
                    <input type="date" id="fecha_ingreso" name="fecha_ingreso" class="input-fieldcontenedor" value="{{ isset($guia->created_at) ? \Carbon\Carbon::parse($guia->created_at)->format('Y-m-d') : 'No disponible' }}" readonly>
                </div>

                <div class="input-groupcontenedor">
                    <label for="orden_servicio" class="input-labelcontenedor">Orden de servicio:</label>
                    <input type="text" id="orden_servicio" name="orden_servicio" class="input-fieldcontenedor" value="{{ $guia->orden_servicio ?? 'No asignado' }}" readonly>
                </div>
            </div>
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr data-id="{{ $detalle_s->id }}">
                                        ...
                                        <!-- Modal Subir Imagen (Bootstrap 4) -->
                                        <div id="modalSubirImagen-{{ $detalle_s->id }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalSubirImagenLabel-{{ $detalle_s->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                <div class="modal-header bg-primary text-white">
                                                    <h5 class="modal-title" id="modalSubirImagenLabel-{{ $detalle_s->id }}">📷 Subir Imagen del Detalle</h5>
                                                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Cerrar">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <div class="modal-body">
                                                    <form id="formSubirImagen-{{ $detalle_s->id }}" enctype="multipart/form-data">
                                                    @csrf
                                                    <div class="form-group">
                                                        <label for="foto-{{ $detalle_s->id }}">🖼️ Imagen</label>
                                                        <input type="file" class="form-control-file" id="foto-{{ $detalle_s->id }}" name="foto" accept=".jpg,.jpeg,.png,.webp" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="descripcion-{{ $detalle_s->id }}">📝 Descripción</label>
                                                        <textarea class="form-control" id="descripcion-{{ $detalle_s->id }}" name="descripcion" rows="3" placeholder="Describe esta imagen..." required></textarea>
                                                    </div>
                                                    </form>
                                                </div>
                                                <div class="modal-footer">
                                                    <button  type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
                                                    <button type="button" class="btn btn-success" onclick="subirImagen({{ $detalle_s->id }})">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modalSubirImagen-{{ $detalle_s->id }}">
                                        ...
                                        <td>
                                            ...
                                            <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modalVerImagen-{{ $detalle_s->id }}">
                                    ...
                                    @if(isset($imagenesProducto[$detalle_s->id]))
                                        ...
                                        <!-- Modal Ver Imagen (Bootstrap 4) -->
                                        <div id="modalVerImagen-{{ $detalle_s->id }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalVerImagenLabel-{{ $detalle_s->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header bg-primary text-white">
                                                    ...
                                                    <div class="modal-body text-center">
                                                        <img src="{{ asset($img->foto) }}" alt="Imagen" class="img-fluid rounded" style="max-height:500px;">
                                                        @if($img->descripcion)
                                                            <textarea class="form-control mt-3" rows="4" readonly>{{ $img->descripcion }}</textarea>
                                                        @endif
                                                    </div>
                                                    <div class="modal-footer">
            ...
            <div class="Contendortecnico">
                ...
                <div class="contenedor-izquierda">
                    ...
                    <div id="contenedor-descripcion" style="text-align: center;">
                        <p>Descripción:</p>
                        <div>
                            <textarea class="contenedordescripcion" id="descripcion_os" placeholder="Escribe la descripción" readonly style="height: 100px; width: 100%; overflow-y: auto; resize: none; padding: 4px;">
                        </textarea>
...
async function subirImagen(detalleId) {
    const modalSelector = '#modalSubirImagen-' + detalleId;
    ...
    try {
        ...
        if (result.success) {
            ...
            Swal.fire({
                ...
                // Después de cerrar la alerta, refresca la sección 2 y abre el acordeón
                localStorage.setItem("seccionActiva", "seccion2");
                localStorage.setItem("acordeonActivo", "acordeon2-collapse-{{ $guia->id }}");
