...
@section('content')
                            ...
                            <div class="tab-content" style="margin-top: -1px">
                                <div class="tab-pane active show" role="tabpanel" id="tab-1"
                                    ...
                                    <div class="table-responsive">
                                        <table
                                            class="table table-striped table-bordered table-hover dataTables-example-facturas_pagados">
                                            <thead>
                                                <tr>
                                                    <th>
                                                    ...
                                                    <th>@can('factura.detalle_pago') Acciones @endcan</th>
                                                </tr>
                                            </thead>
                                            <tbody>
    ...
    <script>
        ...
        var permiso_pago = false;
        var fact_m_table = $('.dataTables-example-facturas_pagados').DataTable({
            "serverSide": true,
            "ajax": {
                url: "{{ route('cobranzas.lista_facturas_pagados_index') }}",
                method: "get",
                data: function(d) {
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('thead input[type="checkbox"]').on('ifChecked ifUnchecked', function(event) {
                ...
                if (event.type === 'ifChecked') {
                    ...
                    getAllIds(function(ids) {
                        ...
                        isUpdatingCheckboxes = true;
                        $('.dataTables-example-facturas_pagados tbody input[type="checkbox"]').iCheck('check');
                        isUpdatingCheckboxes = false;
                    });
                } else {
                    masterChecked = false;
                    ...
                    isUpdatingCheckboxes = true;
                    $('.dataTables-example-facturas_pagados tbody input[type="checkbox"]').iCheck('uncheck');
                    isUpdatingCheckboxes = false;
                }
            });

            // Checkboxes individuales
            $(document).on('ifChecked ifUnchecked', '.dataTables-example-facturas_pagados tbody input[type="checkbox"]', function(event) {
            ...
            fact_m_table.on('draw', function() {
                ...
                $('.dataTables-example-facturas_pagados tbody input[type="checkbox"]').iCheck({
                    checkboxClass: 'icheckbox_square-green',
                    radioClass: 'iradio_square-green',
                });
                ...
                setTimeout(function() {
                    ...
                    $('.dataTables-example-facturas_pagados tbody input[type="checkbox"]').each(function() {
                        var row = $(this).closest('tr');
                        var rowData = fact_m_table.row(row).data();
            ...
            $('#btn-exportar-filtrado').on('click', function(e) {
                ...
                swal({
                    ...
                    $.ajax({
                        url: "{{ route('cobranzas.facturas_exportar_pagadas') }}",
                        method: "POST",
