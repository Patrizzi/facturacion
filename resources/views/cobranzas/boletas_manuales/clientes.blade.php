...
@section('title', 'Pagos Solo Boletas M.')
@section('content')
                                                ...
                                                <div class="row">
                                                    <div class="col-sm-6">
                                                        <div class="form-group">
                                                            <div class="form-group"><label class="small font-weight-bold">Cantidad de Boletas Totales</label>
                                                                <span class="form-control">{{$boletas->count()}}</span></div>
                                                        </div>
                                                        <div class="form-group">
                                                        ...
                                                        <div class="form-group">
                                                            <div class="form-group"><label class="small font-weight-bold">Boletas sin Pagar</label>
                                                                <span class="form-control">{{$bol_sin->count()}}</span></div>
                                                        </div>
                                                        <div class="form-group">
                                                            <div class="form-group"><label class="small font-weight-bold">Boletas en el Mes Actual</label>
                                                                <span class="form-control">{{$bol_mes->count()}}</span></div>
                                                        </div>
                                                    </div>
                                                    <div class="col-sm-6">
                                                        <div class="form-group">
                                                            <div class="form-group"><label class="small font-weight-bold">Boletas Pagadas Totalmente</label>
                                                        ...
                                                        <div class="form-group">
                                                            <div class="form-group"><label class="small font-weight-bold">Monto de Boletas del Mes ({{$moneda_sol->simbolo}})</label>
                                    ...
                                    @foreach ($boletas as $index =>  $bol)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    {{ $bol->moneda->simbolo }}
                                                    {{ number_format($cuotas_all->where('facturacion_id', $bol->id)->sum('monto'), 2) }}
                                                @endif
                                            </td>
                                            <td class="td-center">
                                                @if ($bol->forma_pago_id == 1)
                                                    ...
                                                    @endif
                                                @else
                                                    @if ($cuotas_all->where('facturacion_id', $bol->id)->where('estado', 0)->count() == 0)
                                                        <button id="cancelado" class="btn btn-primary"
                                                            disabled><strong>PAGADO</strong></button>
                                                    @elseif($cuotas_all->where('facturacion_id', $bol->id)->where('estado', 0)->count() < $cuotas_all->where('facturacion_id', $bol->id)->count())
                                            ...
                                            <td>
                                                <a href="{{ route('facturacion.show', $bol->id) }}"
    ...
    <script>
        $(function() {
            ...
            var data = [{
                label: "Contado: " + {{ $boletas->where('forma_pago_id', 1)->count() }},
                data: {{ $boletas->where('forma_pago_id', 1)->count() }},
                color: "#FF6A6A",
            }, {
                label: "Crédito: " + {{ $boletas->where('forma_pago_id', 2)->count() }},
                data: {{ $boletas->where('forma_pago_id', 2)->count() }},
                color: "#6AA2FF",
            }];
