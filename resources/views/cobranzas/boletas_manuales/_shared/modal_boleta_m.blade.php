<div class="modal fade bd-example-modal-lg" id="boleta_show" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document" style="max-width: 900px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ $boleta->codigo_fac }}</h5>
            ...
            <div class="modal-body">
                    ...
                    <div class="col-lg-6">
                        <div class="form-control">
                            ...
                            <div class="text-left" style="font-size: 85% !important">
                                <strong>Señor(es):</strong> <span> {{ $boleta->cliente->direccion }}</span><br>
                                <strong><span>{{ $boleta->cliente->documento_identificacion }}:</span></strong>
                                {{ $boleta->cliente->numero_documento }} <br>
                                <strong>Dirección:</strong><span> {{ $boleta->cliente->direccion }}</span><br>
                                <div style="display: flex;column-gap: 15px">
                                    ...
                                    <div>
                                        <strong>Celular:</strong> <span>{{ $boleta->cliente->celular }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-control">
                            <div class="text-center">
                                ...
                                <div class="text-left" style="font-size: 85% !important">
                                    <div style="display: flex;column-gap: 15px">
                                        <div>
                                            <strong>Orden de Compra:</strong> <span>{{ $boleta->orden_compra }}</span>
                                        </div>
                                        <div>
                                            <strong>Guia Remisión:</strong> <span>{{ $boleta->guia_remision }}</span>
                                        </div>
                                    </div>
                                    <div style="display: flex;column-gap: 15px">
                                        <div>
                                            <strong>Condicion Pago:</strong><span> {{ $boleta->forma_pago->nombre }}</span>
                                        </div>
                                        <div>
                                            <strong>Moneda:</strong> <span>{{ $boleta->moneda->nombre }}</span>
                                        </div>
                                    </div>
                                    <strong>F. Emision:</strong> <span>{{ $boleta->fecha_emision }} </span><br>
                                    <strong>F. Vencimiento:</strong> <span>{{ $boleta->fecha_vencimiento }}</span> <br>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @foreach ($boleta->registros_m as $item => $registros)
                                <tr>
                                    <td>{{ $item + 1 }}</td>
                                    ...
                                    <td style="display: none">
                                        {{ $sub_total = $registros->boleta_i->op_gravada + $registros->boleta_i->op_inafecta + $registros->boleta_i->op_exonerada }}
                                        {{ $sub_total_gravado = $registros->boleta_i->op_gravada }}
                    ...
                    <div class="col-lg-4">
                        ...
                        <div class="form-control" style="font-size: 95%">
                            <div style="display: flex;column-gap: 15px;justify-content: space-between;">
                                ...
                                <div>
                                    {{ $boleta->moneda->simbolo }}{{ number_format($boleta->op_gravada, 2) }}
                                </div>
                            </div>
                            <div style="display: flex;column-gap: 15px;justify-content: space-between">
                                <div>
                                ...
                                <div>
                                    {{ $boleta->moneda->simbolo }}{{ number_format($boleta->op_inafecta, 2) }}
                                </div>
                            </div>
                            <div style="display: flex;column-gap: 15px;justify-content: space-between">
                                <div>
                                ...
                                <div>
                                    {{ $boleta->moneda->simbolo }}{{ number_format($boleta->op_exonerada, 2) }}
                                </div>
                            </div>
                            <div style="display: flex;column-gap: 15px;justify-content: space-between">
                                <div>
                                ...
                                <div>
                                    {{ $boleta->moneda->simbolo }}{{ number_format($igv_p, 2) }}
                                </div>
                            </div>
                            <div style="display: flex;column-gap: 15px;justify-content: space-between">
                                <div>
                                ...
                                <div>
                                    {{ $boleta->moneda->simbolo }}{{ number_format($end, 2) }}
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-footer" style="justify-content: space-between">
                <div class="text-left">
                    <form class="btn" style="text-align: none;padding: 0 0 0 0"
                        action="{{ route('pdf_bol', $boleta->id) }}">
