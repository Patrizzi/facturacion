    ...
    @section('content')
                                                ...
                                                @foreach ($boletas as $index => $bol)
                                                    @if ($bol->estado_pago != 2)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                @if ($bol->forma_pago->id == 2)
                                                                    {{ $cuotas_all->where('boleta_id', $bol->id)->count() }}
                                                                @else
                                                                    1
                                                                @endif
                                                            </td>
                                                            <td>{{ $bol->moneda->simbolo }}
                                                                @if ($bol->forma_pago_id == 2)
                                                                    {{ number_format($cuotas_all->where('boleta_id', $bol->id)->where('estado', 0)->sum('monto'),2) }}
                                                                    <strong>|</strong>
                                                                    {{ $cuotas_all->where('boleta_id', $bol->id)->where('estado', 0)->count() }}
                                                                @else
                                                                    <span
                                                            ...
                                                            <td>{{ $bol->moneda->simbolo }}
                                                                @if ($bol->forma_pago_id == 2)
                                                                    ...
                                                                    <strong>|</strong>
                                                                    {{ $cuotas_all->where('boleta_id', $bol->id)->where('estado', 1)->count() }}
                                                                @else
                                                                    0.00
                                                                @endif
                                                            </td>
        ...
        <script>
            ...
            function pago_boleta(n_boleta) {
                ...
                var only_id_bol = `
                    <input type="hidden" name="id_boleta_m[]" id="id_boleta_` + n_boleta + `" value="` + n_boleta + `">
                `;
                    ...
                    success: function(msg) {
                        ...
                        msg.forEach(function(row, index) {
                            ...
                            var data = `
                                ...
                                `</label>
                                        <input class="form-control" type="hidden" name="numero_boleta_m[]" id="numero_bol_` +
            ...
            function check_lote(num) {
                ...
                if (elemento[0].checked) {
                    ...
                    var only_id_bol = `
                        <input type="hidden" name="id_boleta_m[]" id="id_boleta_` + id_one[1] + `" value="` + id_one[1] +
                        `">`;
                    $('#ids_divs_boleta').append(only_id_bol);
                } else {
